"""VadCLIP-consistent XD-Violence inference and metric helpers."""
...
from common import XD_LABELS, load_clip_feature, process_test_feature, read_csv, test_chunk_lengths


class XDConcatTestDataset(Dataset):
    """The official XD test loader, with 1280D concat-feature validation."""
...
def build_test_loader(csv_path: str, visual_length: int, expected_width: int, num_workers: int) -> DataLoader:
    # The official XD launcher uses batch_size=1, shuffle=False and the
    # DataLoader defaults otherwise.
    return DataLoader(
...
def infer_item(
    ...
    """Match the per-video prediction path in ``VadCLIP/src/xd_test.py``."""
...
def summarize_records(
    ...
    if expected != len(gt):
        raise ValueError(f"XD frame alignment failed: prediction frames={expected}, gt frames={len(gt)}")
    metrics: dict[str, object] = {
        "videos": len(records),
        "snippet_scores": int(len(prob1)),
        "frame_scores": int(expected),
    ...
    # ``xd_test.py`` passes language softmax probabilities, not raw logits, to
    # the official detection-mAP implementation.
...
def run_evaluation(
    ...
    label_path: str,
    vadclip_root: str,
    description: str = "VadCLIP XD residual evaluation",
